...
namespace Services.Implementations
{
    public class UserService : EntityService<User>, IUserService
    {
        ...
        {
            var user = InitPassword(authDto);
            CheckIfAccountExisted(user.UserName, user.Email);
            CreateUser(user);
            SetRoleForUser(user.Id, DefaultRole.User);
            return new BaseResponse<string>(HttpStatusCode.OK, data: "Registered successfully");
        }

        private static User InitPassword(UserInputDto authDto)
        {
            var user = Mapper.Map<User>(authDto);
            user.EncodePassword("Hoangbuu1998");
            return user;
        }

        private void CheckIfAccountExisted(string userName, string Email)
        {
            ...
            if (Contains(u => u.Email.Equals(Email, StringComparison.InvariantCultureIgnoreCase)))
            {
